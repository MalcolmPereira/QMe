/**
 * Name      : com.malcolm.qme.springdata.entity.AnswerOptionMedia.java
 * Date      : 5/3/15
 * Developer : Malcolm
 * Purpose   : Answer Option Media Entity Class
 */
// Generated May 3, 2015 6:11:19 PM by Hibernate Tools 4.3.1
package com.malcolm.qme.springdata.entity;

import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.GeneratedValue;
import static javax.persistence.GenerationType.IDENTITY;
import javax.persistence.Id;
import javax.persistence.Table;

/**
 * AnswerOptionMedia generated by hbm2java
 */
@Entity
@Table(name = "ANSWER_OPTION_MEDIA", catalog = "qme")
public class AnswerOptionMedia implements java.io.Serializable {

	private static final long serialVersionUID = 2587678388163496632L;
	
	private Integer optionMediaId;
	private int optionId;
	private String optionMediaType;
	private byte[] optionMedia;

	public AnswerOptionMedia() {
	}

	public AnswerOptionMedia(int optionId, String optionMediaType) {
		this.optionId = optionId;
		this.optionMediaType = optionMediaType;
	}

	public AnswerOptionMedia(int optionId, String optionMediaType,
			byte[] optionMedia) {
		this.optionId = optionId;
		this.optionMediaType = optionMediaType;
		this.optionMedia = optionMedia;
	}

	@Id
	@GeneratedValue(strategy = IDENTITY)
	@Column(name = "OPTION_MEDIA_ID", unique = true, nullable = false)
	public Integer getOptionMediaId() {
		return this.optionMediaId;
	}

	public void setOptionMediaId(Integer optionMediaId) {
		this.optionMediaId = optionMediaId;
	}

	@Column(name = "OPTION_ID", nullable = false)
	public int getOptionId() {
		return this.optionId;
	}

	public void setOptionId(int optionId) {
		this.optionId = optionId;
	}

	@Column(name = "OPTION_MEDIA_TYPE", nullable = false, length = 50)
	public String getOptionMediaType() {
		return this.optionMediaType;
	}

	public void setOptionMediaType(String optionMediaType) {
		this.optionMediaType = optionMediaType;
	}

	@Column(name = "OPTION_MEDIA")
	public byte[] getOptionMedia() {
		return this.optionMedia;
	}

	public void setOptionMedia(byte[] optionMedia) {
		this.optionMedia = optionMedia;
	}

}
