...
public interface UserQuizSpringDataRepository extends JpaRepository<UserQuizEntity, Long> {
     ...
     * @return UserQuizEntity List
     */
    @Query( value = "SELECT COALESCE(userQuiz.USER_QUIZ_ID, 0) AS USER_QUIZ_ID , COALESCE(userQuiz.USER_ID, 0) AS USER_ID,quiz.QUIZ_ID,quiz.QUIZ_NAME,quiz.QUIZ_DESC,quiz.CAT_ID,userQuiz.QUIZ_START_DATE,userQuiz.QUIZ_END_DATE,userQuiz.QUIZ_USER_SCORE,userQuiz.QUIZ_MAX_SCORE,userQuiz.QUIZ_TOKEN FROM USER_QUIZ userQuiz RIGHT JOIN QUIZ quiz ON userQuiz.QUIZ_ID = quiz.QUIZ_ID WHERE (userQuiz.USER_ID = :userId OR userQuiz.USER_ID IS NULL) \n#pageable\n",
